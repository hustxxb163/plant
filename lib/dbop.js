...
var user_update = function(_id, info, callback) {
  ...
  collection.user.update({_id: formatId(_id)}, {$set: info}, callback);
};

var user_findOne = function(_id, callback) {
  collection.user.findOne({_id: formatId(_id)}, function(err, result) {
    if (err) callback(err);
...
var repo_update = function(_id, info, callback) {
  info.updated_at = new Date();
  collection.repos.update({_id: formatId(_id)}, {$set: info}, callback);
};

var repo_findOne = function(_id, callback) {
  collection.repos.findOne({_id: formatId(_id)}, function(err, result) {
    if (err) callback(err);
    else callback(null, result);
  });
};
